...
/**
 * Manufacturer string for Spoolman vendor `name` (trimmed, casing as in markdown).
 */
...
/** Lowercase dedupe key; pair with `normalizeVendorName` for `vendors.json` `_key` vs `name`. */
export function vendorKey(name) {
  return normalizeVendorName(name).toLowerCase();
}
...
export function buildExport(rows) {
  ...
  for (const r of rows) {
    ...
    const vk = vendorKey(r.manufacturer);
    if (!vendorDisplayByKey.has(vk)) {
      vendorDisplayByKey.set(vk, normalizeVendorName(r.manufacturer));
    }
    if (!seenV.has(vk)) {
      seenV.add(vk);
      vendorOrder.push(vk);
  ...
  const vendorsJson = vendorOrder.map((vk) => ({
    _key: vk,
